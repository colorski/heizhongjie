...
class Desclaimer extends PureComponent {
  constructor (props) {
    super(props)

    this.handleCloseDesclaimerClick = this.handleCloseDesclaimerClick.bind(this);
  }

  render () {
    const { desclaimerShow } = this.props;

    return (
      <div className={desclaimerShow? 'desclaimer_bg' : 'desclaimer_bg_hide'}>
        <div className="desclaimer">
          <div className="wrap">
            <h1>网站免责声明</h1>
            <div className="content">
              <p>本网站所提供的信息，为注册用户的主观言论，仅供参考，请审慎对待。</p>
            ...
            <span className="close" onClick={this.handleCloseDesclaimerClick}></span>
          </div>
        </div>
      </div>
    )
  ...
  handleCloseDesclaimerClick () {
    const action={
      type: 'DESCLAIMER_SHOW',
      desclaimerShow: false,
    }
    this.props.onDesclaimerShow(action);
  }

}

const mapStateToProps = (state) => ({
  desclaimerShow: state.getIn(['global','desclaimerShow'])
})

const mapDispatchToProps = (dispatch) => ({
  onDesclaimerShow(action){
    dispatch(action)
  }
})

export default connect(mapStateToProps, mapDispatchToProps)(Desclaimer);